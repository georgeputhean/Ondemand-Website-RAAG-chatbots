#!/usr/bin/env python
"""
Voice Agent Bot using modern Pipecat architecture.
Supports WebRTC transport with STT, LLM, and TTS pipeline.
"""
...
async def run_bot(transport, business_id: Optional[str] = None):
    ...
    tts = DeepgramTTSService(
        ...
        voice="aura-asteria-en",  # Deepgram Aura voice
    )
...
async def main():
    ...
    parser.add_argument("--transport", default="webrtc", choices=["webrtc"], help="Transport type")
    ...
    if not DEEPGRAM_API_KEY:
        ...
        sys.exit(1)
    # Cartesia no longer needed - using Deepgram for both STT and TTS
    ...
    transport_params = {
        "webrtc": lambda: TransportParams(
            ...
            # vad_analyzer=SileroVADAnalyzer(),  # Disabled for Windows compatibility
        ),
    }

    # Runner arguments - create proper argparse-like object
    class RunnerArgs:
    ...
    runner_args = RunnerArgs(args.transport, args.host, args.port)
